...
:: Obtenir la date du dossier le plus récent dans le répertoire de destination
for /f "delims=" %%a in ('dir /b /ad /o-d /tw "%destination%"') do (
    set "latestFolder=%%a"
)


:: Extraire la date du dossier le plus récent
set "latestDate=!latestFolder:~0,6!"
...
:: Afficher la date actuelle au format YYMMDD
set date=!year!!month!!day!

:: Calculer la différence entre les deux dates en jours
set /a "diffDays=(date - latestDate)"

:: Vérifier si la différence est supérieure à un mois (30 jours)
if !diffDays! gtr 30 (
